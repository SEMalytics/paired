...
class ADRManager {
  constructor(agent) {
    ...
    this.adrRegistry = [];
    this.nextADRId = 1;
    this.trackingFile = path.join(this.agent.config?.projectRoot || process.cwd(), 'core/agents/leonardo/tracking/adr_registry.md');
  }
...
---`).join('\n\n');
  ...
  async updateTracking(result) {
    try {
      const trackingFile = path.join(this.agent.config?.projectRoot || process.cwd(), 'core/agents/leonardo/tracking/adr_history.md');
