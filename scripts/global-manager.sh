#!/bin/bash
# WEE Global Manager Script
# Manages global WEE operations: registry, aggregation, updates
...
# Help function
show_help() {
    echo -e "${BLUE}🌍 WEE Global Manager${NC}"
    echo "===================="
    echo ""
    echo "Usage: wee-global <command> [options]"
    echo ""
    echo "Commands:"
    ...
    echo ""
    echo "Examples:"
    echo "  wee-global registry"
    echo "  wee-global register /path/to/project"
    echo "  wee-global aggregate-learning"
}

# Registry functions
show_registry() {
    echo -e "${BLUE}📊 WEE Project Registry${NC}"
    echo "======================="
    echo ""
    ...
    if [ "$total" -eq 0 ]; then
        ...
        echo -e "${YELLOW}💡 Use 'wee-init' in a project to register it${NC}"
        return 0
    fi
...
register_project() {
    ...
    # Check if project has WEE
    if [ ! -f "$project_path/.weerules" ] && [ ! -d "$project_path/.wee" ]; then
        echo -e "${YELLOW}⚠️  Project doesn't appear to have WEE initialized${NC}"
        echo -e "${YELLOW}💡 Run 'wee-init' in the project first${NC}"
        return 1
    fi
...
aggregate_learning() {
    ...
    local backup_file="$HOME/.wee/memory/global_knowledge_backup_$(date +%Y%m%d_%H%M%S).md"
    ...
    while read -r project_path; do
        if [ -f "$project_path/.wee/memory/ai_memory.md" ]; then
            ...
            grep -E "^(#|##|\*\*|Key|Important|Solution|Pattern)" "$project_path/.wee/memory/ai_memory.md" 2>/dev/null >> "$GLOBAL_MEMORY" || true
            echo "" >> "$GLOBAL_MEMORY"
...
health_check() {
    echo -e "${BLUE}🔍 Checking health of all WEE projects...${NC}"
    ...
    while IFS='|' read -r project_path project_name; do
        if [ -d "$project_path" ] && ([ -f "$project_path/.weerules" ] || [ -d "$project_path/.wee" ]); then
