...
# Safely backs up existing Windsurf configs before WEE installation
...
BACKUP_DIR="$HOME/.wee-backups/windsurf-$(date +%Y%m%d-%H%M%S)"
RESTORE_MANIFEST="$HOME/.wee/backups/restore-manifest.json"
...
check_existing_configs() {
    ...
    if [ -f "$HOME/.weerules" ]; then
        echo -e "${YELLOW}📄 Found: ~/.weerules${NC}"
        configs_found=true
    fi
    
    # Check for Windsurf files
    if [ -f "$HOME/.weefiles" ]; then
        echo -e "${YELLOW}📄 Found: ~/.weefiles${NC}"
        configs_found=true
    fi
    ...
    if [ -d "$HOME/.wee" ]; then
        echo -e "${YELLOW}📁 Found: ~/.wee directory${NC}"
        configs_found=true
    fi
    ...
    cat > "$manifest_file" << EOF
{
  ...
  "backup_reason": "Pre-WEE installation safety backup",
  "backed_up_files": [],
  "shell_config": "$(detect_shell_config)",
...
EOF
    ...
    # Backup .weerules
    if [ -f "$HOME/.weerules" ]; then
        cp "$HOME/.weerules" "$BACKUP_DIR/windsurfrules.backup"
        echo -e "${GREEN}✅ Backed up ~/.weerules${NC}"
        files_backed_up=$((files_backed_up + 1))
    fi
    
    # Backup .weefiles
    if [ -f "$HOME/.weefiles" ]; then
        cp "$HOME/.weefiles" "$BACKUP_DIR/windsurffiles.backup"
        echo -e "${GREEN}✅ Backed up ~/.weefiles${NC}"
        files_backed_up=$((files_backed_up + 1))
    fi
    ...
    # Backup .wee directory if it exists (EXCLUDE backups dir to prevent recursion)
    if [ -d "$HOME/.wee" ]; then
        # Create temp directory for safe backup
        local temp_backup_dir
        temp_backup_dir=$(mktemp -d)
        
        # Copy .wee contents EXCLUDING the backups directory to prevent recursion
        rsync -av --exclude='backups/' "$HOME/.wee/" "$temp_backup_dir/" 2>/dev/null || {
            # Fallback: manual copy excluding backups
            mkdir -p "$temp_backup_dir"
            find "$HOME/.wee" -maxdepth 1 -not -name 'backups' -not -path "$HOME/.wee" -exec cp -r {} "$temp_backup_dir/" \; 2>/dev/null || true
        }
        
        # Move the safe backup to final location
        mv "$temp_backup_dir" "$BACKUP_DIR/windsurf-directory.backup"
        echo -e "${GREEN}✅ Backed up ~/.wee directory (excluding backups to prevent recursion)${NC}"
        files_backed_up=$((files_backed_up + 1))
    fi
    ...
    jq ". += [{
        ...
        \"reason\": \"Pre-WEE installation safety backup\"
...
restore_backup() {
    ...
    if [ -f "$backup_dir/windsurfrules.backup" ]; then
        cp "$backup_dir/windsurfrules.backup" "$HOME/.weerules"
        echo -e "${GREEN}✅ Restored ~/.weerules${NC}"
        restored_count=$((restored_count + 1))
    fi
    
    if [ -f "$backup_dir/windsurffiles.backup" ]; then
        cp "$backup_dir/windsurffiles.backup" "$HOME/.weefiles"
        echo -e "${GREEN}✅ Restored ~/.weefiles${NC}"
        restored_count=$((restored_count + 1))
    fi
    ...
    if [ -d "$backup_dir/windsurf-directory.backup" ]; then
        if [ -d "$HOME/.wee" ]; then
            echo -e "${YELLOW}⚠️  ~/.wee already exists, creating .wee.wee-replaced${NC}"
            mv "$HOME/.wee" "$HOME/.wee.wee-replaced"
        fi
        cp -r "$backup_dir/windsurf-directory.backup" "$HOME/.wee"
        echo -e "${GREEN}✅ Restored ~/.wee directory${NC}"
        restored_count=$((restored_count + 1))
    fi
