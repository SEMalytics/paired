...
**Ready for collaborative development!**`;
  ...
  if (selection === 'Meet the Team') {
    ...
    vscode.env.openExternal(vscode.Uri.parse('https://github.com/SEMalytics/paired'));
  }
...
/**
 * Start PAIRED agents using auto-start-agents.sh
 */
async function startPAIREDAgents() {
  try {
    ...
    const autoStartScript = path.join(pairedDir, 'scripts', 'auto-start-agents.sh');

    if (require('fs').existsSync(autoStartScript)) {
      // Run auto-start-agents.sh
      const { stdout, stderr } = await execAsync(`bash "${autoStartScript}"`, {
