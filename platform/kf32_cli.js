...
/**
 * KnowledgeForge 3.2 Inter-Agent Communication Interface
 *
 * Internal communication system for agents to coordinate with each other.
 * Enables agents to request expertise, delegate tasks, and share context.
 *
 * Philosophy: "Agent-to-Agent Collaboration"
 * - Agents can directly request help from specialists
 * - Structured communication protocols
 * - Context sharing between agents
 * - Reduces Alex coordination overhead for peer collaboration
 */
...
class KnowledgeForge32CLI {
  constructor() {
    this.bridgeUrl = 'ws://localhost:7890';
    this.ws = null;
    this.commandHistory = [];
  ...
  /**
   * Connect to CASCADE Bridge instead of direct orchestrator
   */
  async connectToBridge() {
    try {
      ...
      this.ws.on('open', () => {
        console.log('🌉 Connected to CASCADE Bridge');
      });
      ...
      this.ws.on('error', (error) => {
        ...
        console.log('💡 Make sure CASCADE bridge is running: paired-start');
      });
  ...
  /**
   * Send command to bridge (Alex coordination layer)
   */
  async sendToBridge(command, args) {
    return new Promise((resolve, reject) => {
      if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
        reject(new Error('Bridge not connected. Run: paired-start'));
        return;
      }
  ...
  /**
   * Register inter-agent communication commands
   */
  registerDirectAgentCommands() {
    const agents = [
      { cmd: 'alex', name: 'Alex', role: 'PM', emoji: '👑' },
      { cmd: 'edison', name: 'Edison', role: 'Developer', emoji: '⚡' },
      { cmd: 'leonardo', name: 'Leonardo', role: 'Architect', emoji: '🏛️' },
      { cmd: 'maya', name: 'Maya', role: 'UX Designer', emoji: '🎨' },
      { cmd: 'sherlock', name: 'Sherlock', role: 'QA Specialist', emoji: '🕵️' },
      { cmd: 'vince', name: 'Vince', role: 'Scrum Master', emoji: '🏈' },
      { cmd: 'marie', name: 'Marie', role: 'Data Analyst', emoji: '🔬' }
    ];
    ...
    agents.forEach(agent => {
      this.commands.set(agent.cmd, {
        ...
        role: agent.role,
        emoji: agent.emoji,
        handler: this.handleAgentCommunication.bind(this),
        description: `Request ${agent.role} expertise from ${agent.emoji} ${agent.name}`
      });
    });
  }
   ...
   * Example: Edison calls "kf32 leonardo 'I need architecture review for user auth module'"
   */
  async handleAgentCommunication(targetAgent, requestingAgent, message) {
    try {
      ...
      const response = await this.sendToBridge(`collaborate:${targetAgent}`, communicationMessage);
      return response;
    } catch (error) {
  ...
  /**
   * Handle task-based commands through bridge
   */
  async handleTaskBased(command, agent, args) {
    try {
      const response = await this.sendToBridge(`task:${command}`, { agent, args });
      console.log(response.result || response.message);
      return response;
    } catch (error) {
      console.error(`❌ Failed to execute task ${command}:`, error.message);
    }
  }

  /**
   * Handle context-aware commands through bridge
   */
  async handleContextAware(command, args) {
    try {
      const response = await this.sendToBridge(`context:${command}`, args);
      console.log(response.result || response.message);
      return response;
    } catch (error) {
      console.error(`❌ Failed to execute context command ${command}:`, error.message);
    }
  }

  /**
   * Main CLI entry point - now routes through CASCADE bridge
   */
  async execute(args) {
    if (args.length === 0) {
...
Examples:
  ...
  kf32 review-code core/main.js      # Review specific file
