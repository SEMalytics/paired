/**
 * WEE (Windsurf Evolutionary Ecosystem) Base Agent Class
 * 
...
class BaseAgent extends EventEmitter {
  constructor(orchestrator, config) {
    ...
    // Initialize base systems (async - call manually in production)
    // In tests, this should be called explicitly to avoid hanging promises
  ...
  async processRequest(request, context = {}) {
    ...
    try {
      ...
      // 🔔 NOTIFICATION: Agent started working
      await this.sendNotification('task_started', {
        agent: this.name,
        task: request.substring(0, 100),
        taskId,
      ...
      // 🔔 NOTIFICATION: Analysis phase
      await this.sendNotification('analysis_started', {
        agent: this.name,
        phase: 'analysis',
        taskId
      });
      ...
      // 🔔 NOTIFICATION: Execution phase
      await this.sendNotification('execution_started', {
        agent: this.name,
        phase: 'execution',
      ...
      await this.sendNotification('task_completed', {
        agent: this.name,
        taskId,
        duration,
        success: true,
      ...
      await this.sendNotification('task_failed', {
        agent: this.name,
        taskId,
        error: error.message,
        duration,
  ...
  async loadAgentMemory() {
    // Load agent-specific memory on initialization with robust timeout and fallback
    try {
      // Check if memory manager is available first
      if (!this.memoryManager) {
        console.log(`📚 No memory manager available for ${this.name}, using empty memory`);
        this.agentMemory = [];
        return;
      }
      
      // Add aggressive timeout to prevent infinite hangs
      const memoryPromise = this.getRelevantMemory({}, '', 20); // Reduced limit
      const timeoutPromise = new Promise((_, reject) => 
        setTimeout(() => reject(new Error('Memory loading timeout')), 1500) // Shorter timeout
      );
      
      this.agentMemory = await Promise.race([memoryPromise, timeoutPromise]);
      console.log(`📚 Loaded ${this.agentMemory.length} memory entries for ${this.name}`);
    } catch (error) {
      // More robust error handling
      if (error.message.includes('timeout')) {
        console.warn(`⚠️ Memory loading timed out for ${this.name} - using empty memory`);
      } else {
        console.warn(`⚠️ Failed to load memory for ${this.name}: ${error.message}`);
      }
      console.log(`📚 Initializing ${this.name} with empty memory (fallback)`);
      this.agentMemory = []; // Always fallback to empty memory
    }
  }
  ...
  /**
   * Send notification about agent activity
   */
  async sendNotification(type, data) {
    try {
      // Send to notification system if available
      if (this.notifications && this.notifications.emit) {
      ...
      // Also emit as agent event
      this.emit('notification', { type, data });
      
      // Send system notification for important events
      if (['task_started', 'task_completed', 'task_failed'].includes(type)) {
  ...
  async sendSystemNotification(type, data) {
    try {
      ...
      switch (type) {
        case 'task_started':
          title = `🤖 ${data.agent} Agent`;
          message = `Started: ${data.task}`;
          break;
        case 'task_completed':
          title = `✅ ${data.agent} Agent`;
          message = `Completed task in ${data.duration}ms`;
      ...
      const command = `osascript -e 'display notification "${message}" with title "${title}" ${sound}' 2>/dev/null`;
      await execAsync(command);
      
    } catch (error) {
      // Silently fail - system notifications are optional
    }
  }
