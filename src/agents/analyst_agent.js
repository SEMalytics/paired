...
class AnalystAgent extends BaseAgent {
  constructor(orchestrator, config) {
    ...
    console.log(`🏃 Marie (Analyst) initializing...`);
  }
  ...
  async ensureAnalystDirectories() {
    const baseDir = path.join(process.cwd(), '.wee', 'agents', 'analyst');
    const dirs = [
      'research',
      'competitive',
      'requirements',
  ...
  async saveInsights(insights) {
    const filePath = path.join(process.cwd(), '.wee', 'agents', 'analyst', 'insights', `${insights.id}.json`);
    await fs.writeFile(filePath, JSON.stringify(insights, null, 2));
  }

  async saveResearchReport(report) {
    const filePath = path.join(process.cwd(), '.wee', 'agents', 'analyst', 'reports', `${report.id}.json`);
