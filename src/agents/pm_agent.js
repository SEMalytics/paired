...
const TeamCoordination = require('./pm_agent/modules/team_coordination');
...
class PMAgent extends BaseAgent {
  constructor(orchestrator, config) {
    ...
    console.log(`🎯 Alex (PM) initializing...`);
  }
  ...
  async initializeAgentSystems() {
    if (this.isAgentInitialized) {
      // Silent return to prevent duplicate output
      return;
    }
    ...
    try {
      ...
      try {
        ...
        this.teamCoordination = new TeamCoordination(this);
        // TeamCoordination doesn't need async initialization
        ...
        // Reset initialization flag on module failure
        this.isAgentInitialized = false;
        throw moduleError;
      }
      ...
      this.isAgentInitialized = false; // Reset on failure
      throw error;
    }
  }
